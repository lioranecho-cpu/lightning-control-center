...
START9_MODE = os.environ.get("START9_MODE", "false").lower() == "true"

# On Start9 the UI is served locally — allow all origins; otherwise use explicit list
cors_origins = ["*"] if START9_MODE else os.getenv(
    "LCC_CORS_ORIGINS",
    "https://lcc.satslist.shop,http://localhost:8765"
).split(",")
...
if MOCK:
    try:
        MOCK_DATA = json.load(open(_data_path("data.json")))
    except Exception:
        MOCK_DATA = {}

# ── lncli ─────────────────────────────────────────────────────────────────────

def _lncli_base_args() -> list:
    """Return connection flags for lncli when running inside Start9."""
    if not START9_MODE:
        return []
    args = []
    grpc_host = os.environ.get("LND_GRPC_HOST", "")
    macaroon = os.environ.get("LND_MACAROON_PATH", "")
    cert = os.environ.get("LND_TLS_CERT_PATH", "")
    if grpc_host:
        args += [f"--rpcserver={grpc_host}"]
    if macaroon:
        args += [f"--macaroonpath={macaroon}"]
    if cert:
        args += [f"--tlscertpath={cert}"]
    return args

def run_lncli(*args):
    try:
        timeout = 90 if "sendpayment" in args else 10
        cmd = ["lncli"] + _lncli_base_args() + list(args)
...
def _btc_rpc_auth():
    """Return (user, password) for Bitcoin Core JSON-RPC."""
    cookie = os.environ.get("BTC_COOKIE_PATH", "")
    if cookie and os.path.exists(cookie):
        text = open(cookie).read().strip()
        if ":" in text:
            u, p = text.split(":", 1)
            return u, p
    return os.getenv("RPC_USER", ""), os.getenv("RPC_PASS", "")

def run_bitcoin_rpc(method: str, *params):
    """Call Bitcoin Core JSON-RPC; returns parsed result field."""
    import requests as _req
    if START9_MODE:
        host = os.environ.get("BTC_RPC_HOST", "")
    else:
        h = os.getenv("RPC_HOST", "127.0.0.1")
        p = os.getenv("RPC_PORT", "8332")
        host = f"{h}:{p}"
    if not host:
        raise HTTPException(status_code=503, detail="Bitcoin Core not configured")
    user, pw = _btc_rpc_auth()
    try:
        r = _req.post(
            f"http://{host}",
            json={"jsonrpc": "1.0", "id": "lcc", "method": method, "params": list(params)},
            auth=(user, pw),
            timeout=10,
        )
        data = r.json()
        ...
        raise HTTPException(status_code=503, detail="Bitcoin Core unreachable")
    except _req.exceptions.Timeout:
        raise HTTPException(status_code=504, detail="Bitcoin Core timed out")

def run_bitcoin_cli(*args):
    """Legacy wrapper — redirects to JSON-RPC."""
    method_map = {
        "getmempoolinfo": ("getmempoolinfo", []),
        "getblockchaininfo": ("getblockchaininfo", []),
    }
    if args and args[0] == "estimatesmartfee":
        return run_bitcoin_rpc("estimatesmartfee", int(args[1]))
    if args and args[0] in method_map:
        m, _ = method_map[args[0]]
        return run_bitcoin_rpc(m)
    # fallback: best-effort JSON-RPC
    return run_bitcoin_rpc(args[0], *args[1:]) if args else {}

# ── Loop CLI stub for Start9 ───────────────────────────────────────────────────

LOOP_BIN = "/home/luca/go/bin/loop"

def run_loop(cmd, *args, input_text=None):
    if START9_MODE:
        return ""
    full_cmd = [LOOP_BIN, cmd] + list(args)
    result = subprocess.run(full_cmd, capture_output=True, text=True, input=input_text)
    return result.stdout + result.stderr

# ── Service status helper ──────────────────────────────────────────────────────

def _check_service_active(svc: dict) -> bool:
    if START9_MODE:
        # On Start9 check reachability via bridge address for known services
        unit = svc.get("unit", "")
        if unit == "bitcoin-core-rpc":
            try:
                run_bitcoin_rpc("getblockchaininfo")
                return True
            except Exception:
                return False
        if unit in ("litd", "lnd"):
            try:
                ...
                return False
        # Other services not running in Start9 context
        return False
    else:
        unit = svc.get("unit", "")
        if unit == "bitcoin-core-rpc":
...
def get_node_info():
    if MOCK:
        return MOCK_DATA.get("node", {})
    info = run_lncli("getinfo")
    try:
        data = json.load(open(_data_path("data.json")))
    except Exception:
        data = {}
    return {
        ...
        "uptime_seconds": int(time_module.time()),
        "auto_rebalance_hours": data.get("auto_rebalance_hours", 24),
        "rebalance_amount": data.get("rebalance_amount", 50000),
    }
...
def get_routing(days: int = 30):
    ...
    for e in events:
        ...
        event_date = date.fromtimestamp(ts)
...
def get_mempool():
    if MOCK:
        return MOCK_DATA.get("mempool", {})
    try:
        info = run_bitcoin_rpc("getmempoolinfo")
        fee_info = run_bitcoin_rpc("estimatesmartfee", 6)
        ...
        return {"size_mb": 0, "fee_sat_vbyte": 0, "congestion": "Unknown", "error": "Bitcoin Core not available"}

@app.get("/api/mining")
def get_mining():
    try:
        data = json.load(open(_data_path("data.json")))
        return data.get("mining", {})
    except Exception:
        return {}
...
def get_transactions(limit: int = 10):
    ...
    try:
        ...
        try:
            node_pubkey = run_lncli("getinfo").get("identity_pubkey", "")
        except Exception:
            node_pubkey = ""
    ...
    for tx in transactions:
        ...
        diff = now - t
        if diff < 3600:
            tx["time"] = f"{int(diff / 60)} min ago"
        elif diff < 86400:
            tx["time"] = f"{int(diff / 3600)} hours ago"
        elif diff < 604800:
            tx["time"] = f"{int(diff / 86400)} days ago"
        else:
            tx["time"] = datetime.fromtimestamp(t).strftime("%b %d %Y")
...
def get_system():
    ...
    try:
        ...
        try:
            bitcoin_disk = psutil.disk_usage('/mnt/bitcoind' if START9_MODE else '/mnt/bitcoin')
        except Exception:
        ...
        uptime_secs = int(time_module.time() - boot_time)
    ...
    if START9_MODE:
        services = [
            ...
            {"name": "LND", "desc": "Lightning Network Daemon — manages payment channels", "unit": "lnd"},
    ...
    for svc in services:
        active = _check_service_active(svc)
        svc["active"] = active
        svc["status"] = "active" if active else ("running" if svc["unit"] == "lcc" and START9_MODE else "stopped")
        del svc["unit"]
...
def get_chaninfo(scid: str = ""):
    ...
    return run_lncli("getchaninfo", f"--chan_id={scid}")
...
def rebalance_targeted(body: dict = Body(...)):
    ...
    try:
        ...
        return {"status": "error", "detail": str(he.detail)}
    except Exception as e:
...
def get_accounting(days: int = 365):
    ...
    try:
        ...
        for e in fwd.get("forwarding_events", []):
            ts = int(e.get("timestamp", 0))
            rows.append({"date": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ts)), "type": "routing_income", "amount_sats": int(e.get("fee_msat", 0)) // 1000, "fee_sats": 0, "description": f"{e.get('chan_id_in','')} -> {e.get('chan_id_out','')} ({e.get('amt_in', 0)} sats)", "txid": e.get("timestamp_ns", "")})
...
def rebalance_channels(target_pubkey: str = None):
    ...
    try:
        channels = run_lncli("listchannels")["channels"]
        results = []
        overfull = [c for c in channels if int(c["capacity"]) > 0 and int(c["local_balance"]) / int(c["capacity"]) > 0.80]
        underfull = [c for c in channels if int(c["capacity"]) > 0 and int(c["local_balance"]) / int(c["capacity"]) < 0.20]
        if target_pubkey:
        ...
        for src in overfull:
            for dst in underfull:
                try:
                    settings = json.load(open(_data_path("data.json")))
                except Exception:
                    settings = {}
                amount = min(
                    int(src["local_balance"]) - int(int(src["capacity"]) * 0.50),
                    int(int(dst["capacity"]) * 0.50) - int(dst["local_balance"]),
                    int(settings.get("rebalance_amount", 50000))
                )
                if amount < 1000:
                    continue
                try:
                    ...
                    results.append({
                        "from": src.get("peer_alias", src["remote_pubkey"][:16]),
                        "to": dst.get("peer_alias", dst["remote_pubkey"][:16]),
                        "amount": amount, "status": "failed", "error": str(e)
                    })
...
def get_journal():
    try:
        with open(_data_path("journal.json"), "r") as f:
...
def save_journal(request: Request):
    import asyncio
    journal_path = _data_path("journal.json")
    try:
...
def get_lnbits_settings():
    try:
        ...
        data = {}
    return {"url": data.get("lnbits_url", ""), "invoice_key": data.get("lnbits_invoice_key", "")}

@app.get("/api/pnl")
def get_pnl(period: str = "30d"):
    days = 30 if period == "30d" else 365 if period == "1y" else 9999
    now_s = int(time.time())
    ...
    rebalance_fees = sum(int(p.get("fee_sat", 0)) for p in payments if p.get("status") == "SUCCEEDED" and p.get("payment_request", "").startswith("lnbc") and int(p.get("value_sat", 0)) > 0)
    channels = run_lncli("listchannels")
    ...
    return {"period": period, "routing_fees": routing_fees, "rebalance_fees": rebalance_fees, "open_fees": open_fees, "close_fees": close_fees, "total_costs": total_costs, "net_pnl": net_pnl}
...
def load_nwc_data():
    path = _data_path("nwc_connections.json")
...
def save_nwc_data(data):
    with open(_data_path("nwc_connections.json"), "w") as f:
        json.dump(data, f, indent=2)

@app.get("/api/nwc/connections")
def nwc_list_connections():
    return {"connections": load_nwc_data().get("connections", [])}

@app.post("/api/nwc/generate")
def nwc_generate(body: dict = Body(...)):
    if not _NOSTR_AVAILABLE:
        raise HTTPException(status_code=503, detail="nostr_sdk not installed")
    ...
    try:
        cfg = json.load(open(_data_path("data.json")))
...
def verify_nsec(body: dict = Body(...)):
    if not _NOSTR_AVAILABLE:
        return {"authorized": False, "error": "nostr_sdk not installed"}
    ...
    try:
        ...
        return {"authorized": pubkey_hex == node_pubkey}
    except Exception as e:
...
def get_pending_channels():
    ...
    return {"pending_open": opening, "pending_closing": closing, "total_limbo": int(data.get("total_limbo_balance", 0))}

# ── Loop endpoints (stubbed on Start9) ────────────────────────────────────────

@app.get("/api/loop/monitor")
def loop_monitor():
    if START9_MODE:
        return {"swaps": [], "note": "Loop not available on Start9"}
    import subprocess as _sp, json as _json
    result = _sp.run([LOOP_BIN, "listswaps"], capture_output=True, text=True, timeout=10)
    try:
        raw = _json.loads(result.stdout)
        swaps = []
        for s in raw.get("swaps", []):
            amt = int(s.get("amt", 0))
            ...
            try:
                ts = int(init_time) // 1000000000
                time_str = datetime.fromtimestamp(ts).strftime("%m/%d/%Y %I:%M %p")
            except Exception:
                time_str = ""
            swaps.append({"id": swap_id, "type": swap_type, "amount": amt, "state": state, "cost_server": cost_server, "cost_onchain": cost_onchain, "cost_offchain": cost_offchain, "total_cost": total_cost, "channel": chan_alias, "time": time_str})
        return {"swaps": swaps}
    except Exception:
...
def loop_quote(amt: int):
    if START9_MODE:
        return {"error": "Loop not available on Start9"}
    try:
...
def loop_out(req: LoopOutRequest):
    if START9_MODE:
        return {"success": False, "error": "Loop not available on Start9"}
    try:
        chan_alias = req.scid
        try:
            channels = run_lncli("listchannels")["channels"]
            ...
            pass
        output = run_loop("out", f"--amt={req.amt}", f"--conf_target={req.conf_target}", f"--channel={req.scid}", "--verbose", input_text="y\n")
        if "Swap initiated" in output or "ID:" in output:
            ...
            json.dump(data, open(_data_path("data.json"), "w"))
            ...
            if strategies and not MOCK:
                ...
                for ch in channels:
                    ...
                    if pct <= floor and current_state != "trapped":
                        run_lncli("updatechanpolicy", "--base_fee_msat=0", f"--fee_rate_ppm={trap_ppm}", "--time_lock_delta=40", f"--chan_point={chan_point}")
                        strategies[chan_point]["state"] = "trapped"
                        strategies[chan_point]["trapped_at"] = int(time.time())
                        data["channel_strategies"] = strategies
                        with open(_data_path("data.json"), "w") as f:
                            json.dump(data, f, indent=2)
                    elif pct > floor and current_state == "trapped":
                        run_lncli("updatechanpolicy", "--base_fee_msat=0", f"--fee_rate_ppm={drain_ppm}", "--time_lock_delta=40", f"--chan_point={chan_point}")
...
threading.Thread(target=auto_rebalance_job, daemon=True).start()
threading.Thread(target=drain_trap_worker, daemon=True).start()
...
@app.get("/dashboard")
def dashboard():
    index = os.path.join(_APP_DIR, "index.html")
